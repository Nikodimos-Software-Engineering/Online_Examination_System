...
public class UpdateExamUI extends JFrame {
    ...
    private JButton updateExamButton, cancelButton, addSubjectButton;
    private JSpinner startTimeSpinner, endTimeSpinner;
    private LinkedHashMap<String, Integer> subjectMap = new LinkedHashMap<>();

    public UpdateExamUI(Socket socket, BufferedReader in, PrintWriter out, String username, String examName,
            Runnable onUpdate) {
        this.socket = socket;
        this.in = in;
        this.out = out;
        ...
        JLabel title = new JLabel("Update Exam");
        ...
        updateExamButton = new JButton("Update Exam");
        cancelButton = new JButton("Cancel");
        styleButton(updateExamButton, new Color(76, 175, 80));
        ...
        updateExamButton.addActionListener(e -> handleUpdateExam(onUpdate));
    ...
    private void loadExamData() {
        try {
            out.println("GET_EXAM_DETAILS");
            out.println(examName);
            String response = in.readLine();
            if ("EXAM_DETAILS".equals(response)) {
                examNameField.setText(examName);
                String subjectName = in.readLine();
                subjectComboBox.setSelectedItem(subjectName);
                Date startTime = new Date(Long.parseLong(in.readLine()));
                Date endTime = new Date(Long.parseLong(in.readLine()));
                startTimeSpinner.setValue(startTime);
                endTimeSpinner.setValue(endTime);
            } else {
                JOptionPane.showMessageDialog(this, "Failed to load exam details: " + response, "Error",
                        JOptionPane.ERROR_MESSAGE);
                dispose();
            }
        } catch (IOException e) {
            JOptionPane.showMessageDialog(this, "Connection error: " + e.getMessage(), "Error",
                    JOptionPane.ERROR_MESSAGE);
            dispose();
        }
    }

    private void handleUpdateExam(Runnable onExamAdded) {
        String newExamName = examNameField.getText().trim();
        ...
        if (newExamName.isEmpty() || subjectId == null) {
        ...
        try {
            out.println("UPDATE_EXAM");
            out.println(examName);
            out.println(newExamName);
            ...
            if ("SUCCESS".equals(response)) {
                ...
                JOptionPane.showMessageDialog(this, "Failed to update exam: " + response, "Error",
                        JOptionPane.ERROR_MESSAGE);
            }
        } catch (IOException ex) {
            JOptionPane.showMessageDialog(this, "Connection error: " + ex.getMessage(), "Error",
